...
int main(int argc, char *argv[]) {
    ...
    sdl_tlsf_init_with_size((1 << 20) * 128);  // 128MB
...
//		mem_speed_test(config, base_seed + i);
		memory_stress_test(base_seed);

//	}

	// Check tlsf instance
	int check = sdl_tlsf_check_active_instance();
	if (check == 0) {
		SDL_Log("TLSF Instance is active");
	} else {
		SDL_Log("TLSF Instance is not active");
	}
